...
* **マネー・ローンダリング及びテロ資金供与対策 (AML/CFT) の強化:**  
  ...
  * **FATF（金融活動作業部会）からの国際的な要請:**  
    ...
    * **FATF勧告:** 各国が実施すべき対策の基準。「40の勧告」とテロ資金供与対策に関する「特別勧告」からなる。金融機関等による顧客管理（CDD: Customer Due Diligence、本人確認を含む）、疑わしい取引の届出（STR: Suspicious Transaction Reporting）、記録保存義務などを求めている。  
    * **第4次対日相互審査 (2021年公表):** 日本のAML/CFT体制について、法制度の整備は進んでいるものの、リスクベース・アプローチの実施や実効性の面で課題があると指摘。金融機関等における更なる態勢強化が求められている。  
  * **金融庁「マネロン・テロ資金供与対策に関するガイドライン」による厳格化:** FATF勧告等を踏まえ、金融機関に対し、リスクベースでの継続的な顧客管理（Ongoing Due Diligence）の強化などを求めている（2024年3月までの対応完了を要請）。  
...
* **目的:**  
  ...
  * 本人確認の信頼性レベル（IAL, AAL, FAL）の概念を把握する。  
  * 現行ガイドライン（OpenID F. J.）と最新の改定動向（デジタル庁）を理解する。  
  * 「身元確認結果の活用」等の新しい潮流を知り、自社業務への応用可能性を探る。  
...
OpenID F. J. ガイドラインに基づき、本人確認は以下の2つの要素に分けて考えます。
...
非対面取引における本人特定事項の確認方法として、犯収法施行規則第6条第1項第1号に以下の方法（イ～ワ）が規定されています。
...
* **（ホ）写真付き書類画像＋容貌画像:** 写真付き本人確認書類の画像（表面・裏面・厚み）と本人の容貌画像の送信を受ける。**（eKYCの代表例）**  
* **（ヘ）ICチップ情報＋容貌画像:** 写真付き本人確認書類のICチップ情報と本人の容貌画像の送信を受ける。**（eKYCの代表例）**  
* **（ト）ICチップ情報＋銀行等照会/振込:** 写真付き本人確認書類のICチップ情報送信＋銀行等への顧客情報照会、またはICチップ情報＋顧客名義口座への振込。**（eKYCの代表例）**  
* **（ワ）公的個人認証（署名用電子証明書）:** マイナンバーカードの署名用電子証明書を利用。**（eKYCの代表例）**
...
* **NIST SP 800-63-3 / 行政手続ガイドライン準拠:**  
  * **IAL (Identity Assurance Level):** 身元確認保証レベル (3段階: IAL1〜3)。  
  * **AAL (Authenticator Assurance Level):** 当人認証保証レベル (3段階: AAL1〜3)。  
  * **FAL (Federation Assurance Level):** 認証連携（フェデレーション）の保証レベル (3段階: FAL1〜3)。現行ガイドラインでは詳細な言及は少ない。  
* **DADC IALの参照:**  
...
* マイナンバーカード、運転免許証、パスポート等の特徴を解説。マイナンバーカードへの機能集約の動向にも触れる。

### **4.4. 主な身元確認手法（犯収法との関連含む）**

現行ガイドラインで紹介されている主な手法と保証レベルの目安です。
...
| 身元確認結果の活用 | ≧2 | ≧3 | ※ | 他社確認結果活用(API連携等)。書類撮影不要で利便性高。依拠元による。情報の鮮度に留意。 |  
...
| パスワードレス生体認証(FIDO) | ≧2 | 生体＋所持 | 利便性とセキュリティ両立。フィッシング耐性高。リカバリ課題。 |  
...
* **ホ方式の自動化:** 事業者の審査負荷軽減。  
* **身元確認結果の活用（依拠）:** ユーザーの負担軽減、プライバシー向上。OpenID Connect等の技術活用。FATF勧告の「依拠」の考え方も参考になる。

### **4.7. 事業者・個人情報の留意点**

* 導入時の検討事項（手法選択、UX、システム、周知、体制）を解説。  
* 個人情報保護法の遵守（利用目的特定、安全管理、消去努力、開示請求対応等）の重要性を強調。

## **5\. 最新動向：デジタル庁ガイドライン改定案 (DS-511)**

### **5.1. 改定の背景と位置づけ**

* **背景:** 行政手続オンライン化、マイナンバーカード普及、脅威高度化、国際動向 (NIST SP 800-63-4 Draft, eID Wallet)。  
* **発行主体・対象:** デジタル庁が国の行政機関向けに発行。  
* **位置づけ:** 政府情報システムの**標準ガイドライン (Normative)**。遵守義務あり。民間向けではない。
...
* **「フェデレーション」の明確化・重視:**  
  ...
  * **プロセス:**  
    1. **信頼関係の確立:** RPとIdP間で連携条件（属性情報、保証レベル等）を事前に合意・文書化。  
    2. **設定・登録及び鍵管理:** 安全な連携のためのシステム設定（識別子、暗号鍵交換・登録・管理）。  
    3. **アサーションの授受と検証:** IdPから発行されたアサーションをRPが受け取り、正当性（発行元、非改ざん、宛先、非再利用、有効期限等）を検証。  
    4. **定期的な確認と見直し:** 合意事項の遵守状況を確認し、技術・脅威動向を踏まえ見直し。  
  * **対策基準 (FALレベル設定なし):** NIST SP 800-63-4 2pdのFAL2要件を参考に、一律の対策基準を定義。アサーションインジェクション攻撃等への耐性を要求。  
* **脅威・対策・保証レベルの最新化:** 最新の脅威（高度フィッシング、ディープフェイク等）に対応。  
  * **身元確認保証レベル（IAL）改定案:**
    | レベル | 位置づけ | 属性収集 | 書類検証（いずれか） | 申請者検証（いずれか） |
    | :----- | :----- | :----- | :---------------- | :----------------- |
    | IAL 1 | 影響度「低位」。非対面での券面検査も許容。偽造・改ざんへの簡易的な耐性。 | 任意 | デジタル署名検証、信頼できる情報源への照会、券面物理検査（対面）、**券面物理検査（非対面）** | 容貌確認（対面/非対面）、暗証番号等検証、**確認コード送付** |  
    | IAL 2 | 影響度「中位」。物理的検査（対面）も許容。一定の耐性。 | 任意 | デジタル署名検証、信頼できる情報源への照会、**券面物理検査（対面）** | 容貌確認（対面/非対面）、暗証番号等検証 |
    | IAL 3 | 影響度「高位」。**ICチップ等デジタル検証必須**。偽造・改ざんへの厳格な耐性。 | **電子的読取り** | **デジタル署名検証** | 容貌確認（対面/非対面）、暗証番号等検証 | 
  * **当人認証保証レベル（AAL）改定案:**
...
* **メリット:** 高い信頼性、正確な4情報取得、手続き迅速化の可能性。  
* **課題:** マイナンバーカード普及・暗証番号記憶、NFC対応スマホ、事業者側のシステム対応。  
* **活用シーン:** 口座開設、住所変更、高リスク取引、相続手続き等での活用が期待されます。

### **6.3. IAL, AAL, FAL の理解と適用**

* **IAL (身元確認):** 初回登録時の信頼性。ワ方式(IAL3)、ヘ/ホ方式(IAL2)など、手法によりレベルが異なることを理解し、サービスリスクに応じて選択する。改定案ではICチップ検証が重視される傾向。  
* **AAL (当人認証):** ログイン・取引時の信頼性。パスワード(AAL1)、OTP併用(AAL2)、FIDO/公的個人認証(AAL3)など。**フィッシング耐性**が今後の重要指標（AAL3必須、AAL2推奨）。  
* **FAL (フェデレーション):** ID連携時の信頼性。改定案で詳細な対策基準が示されており、将来的に金融機関がIdPとなる、または外部IdPを利用する際に重要となる概念。  
* **リスクベースアプローチ:** これらの保証レベルを参考に、取引リスクに応じた適切なレベルの本人確認を設計・適用することが求められます（例: 高額送金にはAAL3を要求するなど）。

### **6.4. 身元確認結果の活用（依拠）**

* **現行ガイドラインでの位置づけ:** ユーザー・事業者負担を軽減する「中間的な手法」として紹介。OpenID Connect等の技術活用を想定。  
* **改定案での位置づけ:** 「フェデレーション」の概念に含まれる。連携モデルとして推奨。  
* **金融機関での可能性・具体例:**  
...
* 金融機関としては、これらの動向を注視し、以下の点を検討する必要があります。  
  * 自社サービスのリスク再評価と本人確認レベルの見直し。  
  * ワ方式（公的個人認証）の活用検討。  
  * フィッシング耐性のある当人認証（FIDO、パスキー等）の導入検討。  
  * 将来的な「身元確認結果の活用（フェデレーション）」を見据えたシステム・業務設計。  
