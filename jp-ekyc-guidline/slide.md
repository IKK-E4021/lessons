...
* **脅威・対策・保証レベルの最新化:** 最新の脅威に対応。
     | 保証レベル | 現行 (NIST SP 800-63-3準拠) | 改定案 (DS-511) | 主な変更点・ポイント |
     | :--------- | :-------------------------- | :-------------- | :------------------- |
     | **IAL 1** | 身元確認なし（自己表明） | 影響度「低位」。**非対面での券面検査も許容**。偽造・改ざんへの**簡易的**耐性。 | レベル1でも簡易的な身元確認を行う定義に変更。 |
     | **IAL 2** | 遠隔または対面での身元確認 | 影響度「中位」。デジタル署名検証、信頼できる情報源への照会、または**対面での券面物理検査**が必須。**一定**の耐性。 | 非対面での券面検査はレベル1へ。対面での物理検査はレベル2に残る。 |
     | **IAL 3** | **対面**での身元確認 | 影響度「高位」。**ICチップ等によるデジタル検証（デジタル署名検証）が必須**。偽造・改ざんへの**厳格**な耐性。 | 対面要件がなくなり、ICチップ等の**デジタル的な検証**が必須要件に。 |
     | **AAL 1** | 単要素認証（または多要素認証） | 単要素認証（または多要素認証）。オンライン推測、盗聴・リプレイ耐性。 | 脅威耐性の定義を明記。 |
     | **AAL 2** | 多要素認証 | 多要素認証。**フィッシング耐性（推奨）**。認証器盗用耐性、暗号鍵不正取出し耐性（公開鍵認証の場合）。 | **フィッシング耐性が推奨要件**として追加。 |
     | **AAL 3** | 多要素認証（**耐タンパ性HWトークン**必須） | **公開鍵暗号ベース**を含む多要素認証。**フィッシング耐性（必須）**。認証器盗用耐性、暗号鍵不正取出し耐性。 | 耐タンパ性HW要件がなくなり、**公開鍵暗号ベース**と**フィッシング耐性必須**が要件に。 |
* **リスク評価プロセスの刷新:**
